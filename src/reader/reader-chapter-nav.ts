...
export async function loadChapterList(s: ReaderSession, force = false): Promise<ChapterRef[]> {
  ...
  try {
    ...
    try {
      seriesData = await fetchSeries(permalink, force, s.seriesType() ?? undefined);
    } catch (err) {
      if (s.containerTagPermalink && s.containerTagPermalink !== permalink) {
        try {
          seriesData = await fetchSeries(s.containerTagPermalink, force, s.containerTagType ?? undefined);
          if (seriesData) {
...
export async function gotoAdjacent(s: ReaderSession, direction: "prev" | "next"): Promise<void> {
  ...
  const adj = getAdjacentChapters(s.chapterList(), s.permalink, s.chapterTitle());
  let chapter = direction === "prev" ? adj.prevCh : adj.nextCh;
  if (!chapter && s.seriesPermalink()) {
    const cl = await loadChapterList(s, s.chapterList().length === 0);
    if (cl.length > 0) {
